...
for(let run=1;run<=2;run++){
 ...
 const required=[...Array.from({length:24},(_,i)=>'R'+String(i+1).padStart(2,'0')+' '),...Array.from({length:25},(_,i)=>'P2A'+String(i+1).padStart(2,'0')+' '),...Array.from({length:36},(_,i)=>'P2B'+String(i+1).padStart(2,'0')+' ')];
 ...
 console.log('RUN '+run+': '+names.length+' test cases PASS; 24/24 Phase 1 + 25/25 Phase 2A + 36/36 Phase 2B requirements mapped to actual runner names.');
}
