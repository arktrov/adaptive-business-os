...
test('R20 Clean migration and repeat execution',async()=>{await migrate(db.pool);const tables=(await db.pool.query("SELECT tablename FROM pg_tables WHERE schemaname='public'")).rows.map(x=>x.tablename);for(const t of ['artifacts','evidence_records','content_jobs','job_state_transitions','business_voice_profiles','business_music_assets','business_music_preferences'])assert(tables.includes(t));assert.deepEqual((await db.pool.query('SELECT name FROM schema_migrations ORDER BY name')).rows.map(r=>r.name),(await readdir(new URL('../db/migrations/',import.meta.url))).filter(n=>n.endsWith('.sql')).sort())});
