...
export class LocalScriptProvider {
 ...
 async execute(input,context){this.calls++;
  ...
  const a=captureScriptResponse(data,{contract_version:input.version,request_id:'synthetic',run_id:context.run_id,request_hash:context.input_hash,config:this.describe()});await context.persistResponse(a);
