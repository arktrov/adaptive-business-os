...
export const scriptSchemaV2=structuredClone(scriptSchema);
const beat=object({visual_beat_id:string,narration_segment_id:string,duration_target:{type:'number'},claim_ids:{type:'array',items:string},visual_type:{type:'string',enum:visualTypes},visual_classification:{type:'string',enum:visualClasses},visual_intent:string,concrete_visual_brief:string,motion_intent:string,transition_intent:string,factual_sensitivity:string,source_requirement:string,rights_requirement:string,disclosure_requirement:string,generation_eligibility:{type:'string',enum:['NOT_ELIGIBLE','ELIGIBLE_AFTER_REVIEW']}});
scriptSchemaV2.properties.scripts.items.properties.visual_beats={type:'array',items:beat};scriptSchemaV2.properties.scripts.items.required.push('visual_beats');
export class OpenAIScriptProvider {
 ...
 async execute(input,context){
  ...
  let response;try{response=await this.transport('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:'Bearer '+this.#credential,'Content-Type':'application/json'},signal:AbortSignal.timeout(240000),body:JSON.stringify({model:this.config.model,store:false,max_output_tokens:this.config.max_output_tokens,reasoning:{effort:this.config.reasoning_effort},tools:[],input:[{role:'system',content:input.prompt},{role:'user',content:JSON.stringify(input)}],text:{format:{type:'json_schema',name:'scoped_script',strict:true,schema:['script-production-input/2.0','script-production-input/2.1','script-production-input/2.2'].includes(input.version)?scriptSchemaV2:scriptSchema}}})})}catch{this.#meta.duration_ms=Date.now()-start;throw Error('SCRIPT_PROVIDER_TRANSPORT_FAILURE')}
  ...
  const artifact=captureScriptResponse(data,{contract_version:input.version,request_id:this.#meta.provider_request_id,config:this.describe(),request_hash:context.input_hash,run_id:context.run_id,credential:this.#credential});
