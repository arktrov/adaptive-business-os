import {realizationReference} from './claim-realization.js';
import {resolveScriptProposal,equivalentText,resolveClaimUsage} from './claim-usage.js';
import {validateBeats,beatTimeline} from './visual-beats.js';
...
export function validateProductionInput(i){
 assertSafe(i);if(i.version==='script-production-input/2.2')need(text(i.realization_style_profile)&&Array.isArray(i.claim_realizations),'REALIZATION_CATALOG_REQUIRED');need(['script-production-input/1.0','script-production-input/2.0','script-production-input/2.1','script-production-input/2.2'].includes(i.version)&&['SHORT','LONG','BOTH'].includes(i.format),'INVALID_SCRIPT_INPUT');
...
export function validateScriptProposal(input,proposal){
 validateProductionInput(input);assertSafe(proposal);if(['script-production-input/2.1','script-production-input/2.2'].includes(input.version))proposal=resolveScriptProposal(input,proposal).proposal;
 ...
 for(const script of proposal.scripts){
  need(Object.keys(script).every(k=>(['script-production-input/2.0','script-production-input/2.1','script-production-input/2.2'].includes(input.version)?['format','segments','visual_beats']:['format','segments']).includes(k))&&Array.isArray(script.segments)&&script.segments.length>=3,'SCRIPT_SCHEMA_INVALID');
  ...
  for(const seg of script.segments){
   ...
   if(seg.claim_id===null){need(seg.usage_type==='NONE'&&input.production_policy.neutral_phrases.includes(seg.text),'UNSUPPORTED_FACTUAL_ASSERTION')}
    ...
    need(['script-production-input/2.1','script-production-input/2.2'].includes(input.version)?resolveClaimUsage(input,seg)===seg.usage_type:seg.text===realization(input,seg.claim_id,seg.usage_type),'UNSUPPORTED_OR_UNQUALIFIED_PROSE');
  ...
  if(['script-production-input/2.0','script-production-input/2.1','script-production-input/2.2'].includes(input.version))validateBeats(input,script,segmentDuration);
 ...
 return {status:'PASS',validator_version:input.version==='script-production-input/2.2'?'script-contract/2.2':['script-production-input/2.1','script-production-input/2.2'].includes(input.version)?'script-contract/2.1':['script-production-input/2.0','script-production-input/2.1','script-production-input/2.2'].includes(input.version)?'script-contract/2.0':scriptProcessingVersion,scope_hash:input.scope.content_scope_hash,publication_hold:true,policy_snapshot_refs:input.scope.policy_snapshot_refs,checks:['closed_world_text','qualifiers','evidence','policies','format','duration','visual_classification','hold'],human_creative_review_required:true};
}
...
export function buildDrafts(input,proposal,identity,metadata){
 if(['script-production-input/2.1','script-production-input/2.2'].includes(input.version))proposal=resolveScriptProposal(input,proposal).proposal;
...
export function buildProductionPackage(input,draft,id){
 ...
 let timeline=draft.segments.map(s=>{const external=['OFFICIAL_SOURCE_MEDIA','STOCK_OR_LICENSED_MEDIA'].includes(s.visual_type);const refs=s.claim_id?input.evidence_references[s.claim_id]:[];
